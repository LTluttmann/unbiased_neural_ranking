# NOTE: this is a fork of keras batch normalization layer as of tensorflow v. 2.9
# only change is to replace tf.nn.moments with an implementation that supports masking
...
class BatchNormalizationBase(Layer):
  ...
  *after having been trained on data that has similar statistics as the
  ...
  def _calculate_mean_and_var(self, inputs, reduction_axes, keep_dims):
    ...
    if self.mask:
        mask = tf.not_equal(inputs,self.mask)
        inputs = tf.ragged.boolean_mask(inputs, mask)
    mean = tf.math.reduce_mean(inputs, reduction_axes, keepdims=keep_dims)
    var = tf.math.reduce_variance(inputs, reduction_axes, keepdims=keep_dims)
    return mean, var
